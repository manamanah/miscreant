...
class LoseFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding.deadHeroImg.background = if (result.heroType == Hero.viking) resources.getDrawable(R.drawable.viking_dead, null)
                                        else resources.getDrawable(R.drawable.archer_dead, null)

        binding.heroName.text = result.heroName

        val currentDeck = if (result.accomplishedDecks > 0) result.accomplishedDecks - 1 else 0
        val deckPoints = currentDeck.times(resources.getInteger(R.integer.deckMultiplier))
