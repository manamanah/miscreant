...
class HighscoreRepository(context: Context){
    ...
    private var highscoreDatabaseDao: HighscoreDatabaseDao = HighscoreDatabase.getInstance(context).highscoreDatabaseDao
    ...
    private var insertAsynTask : AsyncTask<Highscore, Unit, Unit>? = null
    ...
    fun insert(highscore: Highscore){
        insertAsynTask = InsertAsyncTask(highscoreDatabaseDao).execute(highscore)
    }
    ...
    fun cancel(){
        if (insertAsynTask?.status != AsyncTask.Status.FINISHED){
            insertAsynTask?.cancel(true)
        }
    ...
    private class InsertAsyncTask(val highscoreDao: HighscoreDatabaseDao) : AsyncTask<Highscore, Unit, Unit>(){
    ...
    private class DeleteDifficultyList(val highscoreDao: HighscoreDatabaseDao) : AsyncTask<String, Unit, Unit>(){
    ...
    private class DeleteButBestList(val highscoreDao: HighscoreDatabaseDao) : AsyncTask<String, Unit, Unit>(){
